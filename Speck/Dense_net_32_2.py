...
def make_net(num_filters=32, num_outputs=1, d1=64, d2=64, ks=3,depth=5, reg_param=0.0001, final_activation='sigmoid'):
    ...
    rs = Reshape((2, 32))(inp)
    perm = Permute((2, 1))(rs)
    conv=Conv1D(24, kernel_size=1, padding='same')(perm)
    x=bn_relu(conv)
    ...
    x = dense_block(x, 12,3, 2)
    #x= bn_relu (x)
    x = transition_block(x, 50)
    x = dense_block(x, 10,2, 2)
    #x= bn_relu (x)
    x = transition_block(x, 50)
    x=bn_relu(x)
    flat1 = Flatten()(x)
    ...
    out = Dense(num_outputs, activation=final_activation, kernel_regularizer=l2(reg_param))(flat1)
