...
class Speck32_64(object):
    ...
    def make_data(self,data_len: int,file='',diff=0x00400000):
        if len(file)==0:
          Y = np.frombuffer(urandom(data_len), dtype=np.uint8)
          plaintext0=np.frombuffer(urandom(4*data_len), dtype=np.uint32)
        else:
          inp=open(file,'rb')
          Y = np.frombuffer(inp.read(data_len), dtype=np.uint8)
          plaintext0=np.frombuffer(inp.read(4*data_len), dtype=np.uint32)
        Y = Y & 1
        ...
        if len(file)==0:
          ...
          plaintext1[Y==0]=np.frombuffer(inp.read(4*num_rand_samples), dtype=np.uint32)
          keys=np.frombuffer(inp.read(8*data_len), dtype=np.uint64)
...
X,Y=speck.make_data(10**7,'/content/drive/MyDrive/gamma500.bin')
X_eval,Y_eval=speck.make_data(10**6,'/content/drive/MyDrive/gamma500.bin')
